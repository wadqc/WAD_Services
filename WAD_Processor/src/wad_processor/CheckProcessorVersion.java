...
public class CheckProcessorVersion {
    ...
    public void CheckVersion(Connection iqcConnection, String version){
        ...
        try {
            ...
            passedCheck = true;
                ...
                log.info("Processor versie:"+version);
            } else {
                log.fatal("Database major versie: "+dbVersion+" komt niet overeen met Processor major versie:"+version);
                log.fatal("Processor wordt niet gestart");
            }
    ...
    public static void UpdateVersion(Connection dbConnection, String version){
        ...
        String sqlStatement = "UPDATE config SET value='"+version+"', date_modified='"+dateFormat.format(date)+"'  WHERE property='Version_Processor'";            
